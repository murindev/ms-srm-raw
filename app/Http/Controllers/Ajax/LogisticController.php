    ...
    public function busyHours($master)
    {
        ...
        foreach ($volume as $k => $v) {
            ...
            if ($v['lid_id']) {
                ...
                $this->lid[$v['lid_id']]['acts'][] = Act::where('id',$v['parent'])->get()->toArray();
            }
        }
        return $vol;
    }
...
//            dump($act);
            ...
            $cust =  Customer::where('lid_id',$act['parent'])->first();
            $customer = $cust ? $cust->toArray(): [];
/*
            if($act['address']){
                $address = $act['address'];
            } else {
...
//        $this->ee = ImplementEquipment::where('date','2019-05-08')->where('master',4)->get();
    ...
    public function printMasterSchedule(){



        $shedule = json_decode($this->getByGeo());


        $masters = (array)$shedule->schedule;
        $this->helpers();

        $date =  \request()->get('date');
        $master = '';
        foreach ($masters as $k => $masterData) if($k == (int)\request()->get('master')) {
            $master = $masterData;
        }
