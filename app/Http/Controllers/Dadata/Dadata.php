...
/*        $flat = \request()->get('flat');
    ...
    public function searchByAddress(){
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://nominatim.openstreetmap.org/search?q=' .\request()->get('query'). '&format=json');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_TIMEOUT, 60);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $this->headers);
        curl_setopt($ch, CURLOPT_USERAGENT, "Mozilla/5.0 (Windows; U; Windows NT 5.1; ru-RU");
        ...
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, '{ "q": "' . \request()->get('query') . '", "format": "json"');
