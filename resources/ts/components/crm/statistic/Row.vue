<template>
    <div>
        ...
        <Modal className="statistic-modal" v-model="modal" :closeOnMask="false" style="left: 60%">
                    ...
                    <td colspan="1"><!--Ю/Ф-->
                        <template v-if="row.customer">
                            <span class="h-tag " :class="row.customer.status ? 'h-tag-yellow':''"
                                  v-html="row.customer.status === 2 ? 'Ю':'Ф'" style="margin-left: 12px;"/>
                        </template>
                    </td>
...
// EOF ACT
        ...
        created() {
            ...
            bus.$on('openRowModal',(val: boolean) => {
                this.modal = val
            })
