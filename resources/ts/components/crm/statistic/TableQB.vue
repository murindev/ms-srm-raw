<template>
    <div>
        <DateFullRangePicker class="roof" style="left: 235px" v-model="period" :layout="datePicker.layout"
                             :startWeek="datePicker.startWeek" @input="filterDataPeriod()"/>

        <Button v-if="Object.keys(oneClickRow).length" class="settings roof" style="left: 620px" color="primary"
                icon="h-icon-top"
                @click="editRow(oneClickRow)">
            Редактировать {{oneClickRow.id}}
        </Button>
        <!--$store.dispatch('lidStatisticEditRow',{ id: lidStatisticEditClonedRow, flag: 'open', manager: manager })-->
        ...
        <div class="statistic-table-container" @scroll="onScroll">
            <table class="statistic-table">
                ...
                <tr>
                    <th v-for="(obj,key) in tableHead" v-if="['period','time'].indexOf(obj.key) === -1" :class="obj.class"  :style="theadStyleNth2(obj)">

<!--Статусы-->
                        <input v-if="obj.key === 'id' && getShowCols(obj.key)" type="text" v-model.number="obj.v" @keyup="filterData()">

                        <DatePicker v-else-if="obj.key === 'date' && getShowCols(obj.key)" v-model="obj.v" :placeholder="'Дата'" @input="filterData()" @clear="obj.v = null, filterData()"/>

                        <Select v-else-if="obj.key === 'site' && getShowCols(obj.key)" v-model="obj.v" :datas="h.companies" keyName="id" titleName="name" @change="filterData()"/>

                        <Select v-else-if="obj.key === 'manager' && getShowCols(obj.key)" v-model="obj.v" :datas="obj.select()" keyName="id" titleName="name" @change="filterData()"/>

                        <Select v-else-if="obj.key === 'call_status' && getShowCols(obj.key)" v-model="obj.v" :datas="callStatus" keyName="id" titleName="name" @change="filterData()"/>

                        <Select v-else-if="obj.key === 'app_status' && getShowCols(obj.key)" v-model="obj.v" :datas="appStatus" keyName="id" titleName="name" @change="filterData()"/>


                        <input v-else-if="obj.key === 'cause' && getShowCols(obj.key)" type="text" v-model="obj.v" @keyup="filterData()">
<!--Клиент-->
                        <input v-else-if="obj.key === 'phone' && getShowCols(obj.key)" type="text" v-model="obj.v"  style="width: 100%;" @keyup="filterData()">

                        <Select v-else-if="obj.key === 'customer_status' && getShowCols(obj.key)"  v-model="obj.v" :datas="customer_status" keyName="id" titleName="short"  @change="filterData()"/>

                        <input v-else-if="obj.key === 'name' && getShowCols(obj.key)" type="text" v-model="obj.v"  @keyup="filterData()" style="width: 100%;">

                        <input v-else-if="obj.key === 'address' && getShowCols(obj.key)" type="text" v-model="obj.v" @keyup="filterData()">
                        <input v-else-if="obj.key === 'organization' && getShowCols(obj.key)" type="text" v-model="obj.v" @keyup="filterData()">

                        <input v-else-if="obj.key === 'email' && getShowCols(obj.key)" type="text" v-model="obj.v"  @keyup="filterData()">
<!--Обьект-->
                        <!--Тип обьекта-->
                        <Select v-else-if="obj.key === 'property_type' && getShowCols(obj.key)" v-model="obj.v" :datas="act_propertyType" keyName="id" titleName="name"  @change="filterData()"/>
                        <!--Предмет работ-->
                        <Select v-else-if="obj.key === 'pest' && getShowCols(obj.key)" v-model="obj.v" :datas="h.pests_asc" keyName="id" :filterable="true"
                                titleName="name" style="min-width: 190px; margin-right: 4px;"  @change="filterData()"/>
...
<!--Финансы-->
                        <!--Нал_сумма-->
                        <input type="text" v-else-if="obj.key === 'cash' && getShowCols(obj.key)"  v-model="obj.v" @keyup="filterData()">
                        <!--**Нал tag-->
<!--                        <Checkbox v-else-if="obj.key === 'cash_tag' && getShowCols('cash')"  v-model="obj.v"  @change="filterData()"/>-->
                        <!--Б_н_сумма-->
                        <input type="text" v-else-if="obj.key === 'cashless' && getShowCols(obj.key)"  v-model="obj.v" @keyup="filterData()">
                        <!--**Безнал tag-->
<!--                        <Checkbox v-else-if="obj.key === 'cashless_tag' && getShowCols('cashless')"  v-model="obj.v" @change="filterData()"/>-->
                        <!--**Карта-->
<!--                        <Checkbox v-else-if="obj.key === 'card_payment' && getShowCols(obj.key)" trueValue="1" falseValue="0"  v-model="obj.v"   @change="filterData()"/>-->
                        <input v-else-if="obj.key === 'card_payment' && getShowCols(obj.key)" type="checkbox" v-model="obj.v" @change="filterData()" style="display: block;">
                        <!--Мастер+-->
                        <input type="text" v-else-if="obj.key === 'overpay' && getShowCols(obj.key)"  v-model="obj.v"  @keyup="filterData()">
                        <!--Субчику-->
                        <input type="text" v-else-if="obj.key === 'contractor_fee' && getShowCols(obj.key)"  v-model="obj.v" @keyup="filterData()">
...
<!--                        <Select v-else-if="obj.key === 'card_payment' && getShowCols(obj.key)" v-model="obj.v" :datas="cardPayment" keyName="value" titleName="name"></Select>-->
                        ...
                        <input v-else-if="obj.key === 'contract' && getShowCols(obj.key)" type="text" v-model="obj.v" style="width: 100%;" @keyup="filterData()">
                        <DatePicker v-else-if="obj.key === 'implement_date' && getShowCols(obj.key)"  v-model="obj.v" format="YYYY-MM-DD"
                                    style="width: 100%;" @input="filterData()"></DatePicker>
<!--                        <input v-else-if="obj.key === 'implement_date' && getShowCols(obj.key)" type="text" v-model="obj.v" style="width: 100%;" @keyup="filterData()">-->
                ...
                <tbody>
                <tr
                    v-for="(row, rowKey) in statistic_table"
                    :id="'tr'+row.id"
                    :class="{active: row.id === oneClickRow.id}"
                    :style="row.editing ? 'background: #ff000014' : ''"
                    @dblclick="editRow(row)"
                    @click="prepareForMobileButtonOpen(row);"
                >
                    <!--row.call_status ? callStatus.find(o => o.id === row.call_status).class : '',-->
                        ...
                        <td v-if="th.key === 'customer_status'">
                            <div v-if="getShowCols('customer_status')">
                                <template v-if="row.customer_id">
                                    ...
                                    <span v-if="row.customer && row.customer.status === 1" class="h-tag">Ф</span>
                                </template>
...
<!--                                        <Poptip content="Переход на страницу создания коммерческих предложений. Продолжить?" @confirm="makeCommercialMail(row.customer.email,row)">-->
                                            <!--Предложение уже было отправлено! Отправить еще одно?', 'warn'-->
                                            <Button size="xs">{{row.customer.email}}</Button>
<!--                                        </Poptip>-->
                                    </div>
                                    <div v-if="row.customer.emails">
                                        <Poptip content="Переход на страницу создания коммерческих предложений. Продолжить?"
                                            ...
                                            <Button :color="row.offer_sent ? 'primary': 'blue'" class="h-icon-outbox" size="xs"> {{row.customer.emails}}</Button>
                                        </Poptip>
                                    </div>
                                </template>
...
<!--Финансы-->
                        <!--cash 1-->
                        <td v-if="th.key === 'cash'" class="act-cell" colspan="1" style="white-space: nowrap">
                            <div v-if="getShowCols('cash')">
                                <template v-if="row.volumes && row.volumes.length">
                                    <span class="h-tag h-tag-primary">{{sumVolumesCash(row.volumes)}}</span>
                                </template>
                                <template v-else-if="row.act_id && row.act">
                                    <span class="h-tag h-tag-red">
                                        {{Number(row.act.cash).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$& ').slice(0, - 3)}}
                                    </span>
                                </template>
                                <template v-else>
                                    <span class="h-tag" v-if="row.cash">
                                ...
                                <!--
                                                                <template v-if="row.act">
                                                                    <span v-if="row.act.cash" class="space-between no-wrap">
                                                                       // volume:cash && cash_tag || act:price_fact
                                        <template>
                        ...
                        <!--cashless 1-->
                        <td v-if="th.key === 'cashless'"  class="act-cell" colspan="1" style="white-space: nowrap">
                            <div v-if="getShowCols('cashless')">

                                <template v-if="row.volumes && row.volumes.length">
                                    <span class="h-tag h-tag-primary">{{sumVolumesCashless(row.volumes)}}</span>
                                </template>
                                <template v-else-if="row.act_id && row.act">
                                    <span class="h-tag h-tag-primary">
                                        {{Number(row.act.cashless).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$& ').slice(0, - 3)}}
                                    </span>
                                </template>
                                <template v-else>
                                    <span class="h-tag" v-if="row.cashless">
...
<!--                                    {{Number(row.cash) + Number(row.cashless)}}-->
                        ...
                        <td v-if="th.key === 'implement_date'">
                            <div v-if="getShowCols('implement_date')">
                                <template >
                                    <template v-if="row.act && row.act.hasOwnProperty('implements')">
                                        <template  v-for="implement in row.act.implements">
                                            <a target="_blank" v-if="implement.start_date" :href="`/layout/lid/${row.lid_id}/edit?open_act=${row.act.id}`">
                                                <Tooltip :content="implement.master ? h.users[implement.master].name : ''">
                                                <Button size="s" :color="row.act.finished ? 'primary' : 'gray'" >
                                                    {{implement.start_date}} - {{implement.end_date}} - {{implement.master}}
                                                </Button>
                                                </Tooltip>
                                            </a>
                                        </template>
...
<script lang="ts">
    ...
    export default class CrmStatisticTableQB extends Vue {
// Props
        ...
        public datePicker: any = {
            startWeek: 1,
            layout   : ['year', 'quarter', 'month', 'week', 'customize'],
        }
        public period = {
            start:false,
            end: false,
...
// Statistic
        @Statistic.State('queryBuilder') queryBuilder!: ICrmStatistic[]
        @Statistic.Action('initTable') fetchQueryBuilder!: (payload:string) => void
        ...
        @Statistic.Getter('getTable') statistic_table!: ICrmStatistic[]

        // @Action
        @Statistic.Action('initTable') statistic_initTable!: (search: string) => void
        @Statistic.Action('fetchConcatTable') statistic_fetchConcatTable!: (search: string) => void
...
// table
       ...
       async filterDataPeriod(){
        ...
        public tableHead = [
            {
                key: 'period', name: 'period', short: 'id', width: 70, v: null, d: null, colspan: 0, show: false, q:'&filter',
            },
            {
                key: 'id', name: 'ID', short: 'id', width: 70, v: null, d: null, colspan: 1, show: true, q:'&filter', alt: 'row',
                f: function (el: ICrmStatistic) {
                    return el.id === this.v;
                },
            },
            {
                key: 'date', name: 'Дата', short: 'Д', width: 150, v: null, d: null, colspan: 1, show: true, q:'&filter', alt:'starts_after',
                f: function (el: ICrmStatistic) {
                    return  this.v;
            ...
            {
                key: 'site', name: 'Сайт', short: 'С', width: 150, colspan: 1, show: true, q:'&filter',
                select: () => {
            ...
            {
                key: 'cause', name: 'Комментарий', short: 'К', width: 150, colspan: 1, v: '', d: '', show: true, q:'&filter',
                f: function (el: ICrmStatistic) {
...
// Клиент
            ...
            {
                key: 'address', name: 'Адрес', short: 'А', width: 150, colspan: 1, class: 'customer-thead', max: true, v: '', d: '', show: true,  q:'&filter',
                f: function (el: ICrmStatistic) {
            ...
            {
                key: 'organization', name: 'Организация', short: 'О', width: 150, colspan: 1, class: 'customer-thead', max: true, v: '', d: '', show: true, q:'&filter',
                f: function (el: ICrmStatistic) {
...
// Финансы
        ...
        sumVolumesCash(volumes:IVolume[]){
           return  volumes.reduce((sum,obj) => {
                return sum + Number(obj.price_fact)
            },0).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$& ').slice(0, - 3)
        }
        sumVolumesCashless(volumes:IVolume[]){
           return  volumes.reduce((sum,obj) => {
                return sum + Number(obj.price_standard)
            },0).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$& ').slice(0, - 3)
        }
...
/*         filterData() {
        ...
        async filterData(){
           ...
           this.tableHead.forEach(th => {
            ...
            await this.statistic_initTable(this.getString);
            if(this.getString === '?sort=-date'){
                bus.$emit('showTableFilterDetachBtn', false)
            } else {
                bus.$emit('showTableFilterDetachBtn', true)
            }
        ...
        oneClickRow: ICrmStatistic = {}
        prepareForMobileButtonOpen(row: ICrmStatistic){
            this.oneClickRow = row

            console.log(this.oneClickRow);
        }
...
// METHODS
        ...
        @Statistic.State('page') page!: number
        @Statistic.State('last_page') last_page!: number
        pageNr: number = 1
        async onScroll(event:any){
            ...
            if(container.scrollHeight <= container.offsetHeight + container.scrollTop) {
                ...
                if(this.page < this.last_page && this.page === this.pageNr){
                    ...
                    await this.statistic_fetchConcatTable(this.getString + '&page='+ (this.page + 1));
                }
            }
        }
        ...
        created() {
            ...
            this.statistic_initTable(this.getString);
