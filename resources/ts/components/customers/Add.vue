    ...
    <div class="customer-add">
        <div class="customer-add-insert">
            ...
            <Form :label-width="110">
                <FormItem label="Поиск организации" v-if="table.status === 2" class="customer-add-insert_formItem">
...
<!--                </FormItem>-->
                <FormItem label="Статус">
                    <Select v-model="table.status" titleName="name" keyName="id" :datas="status" @change="clearTable"/>
                </FormItem>
                <FormItem v-if="table.status === 2" label="Наименование краткое"><input type="text" v-model="table.organization" ></FormItem>
                <FormItem v-if="table.status === 2" label="Наименование"><textarea class="txt100" v-model="table.organization_long" /></FormItem>
                <FormItem v-if="table.status === 2" label="Руководитель"><input type="text" v-model="table.management_name" ></FormItem>
                <FormItem v-if="table.status === 2" label="Должность"><input type="text" v-model="table.management_post" ></FormItem>
                ...
                <FormItem  v-if="table.status === 2" label="Адрес для договора"><textarea class="txt100" v-model="table.address_source"/></FormItem>
                <FormItem  v-if="table.status === 2" label="ZIP" :required="true"><input type="text" v-model="table.zip" ></FormItem>
                <FormItem  v-if="table.status === 2" label="Регион" :required="true"><input type="text" v-model="table.region" ></FormItem>
                ...
                <FormItem  v-if="table.status === 2" label="ОГРН"><input type="text" v-model="table.ogrn" ></FormItem>
                <FormItem  v-if="table.status === 2" label="ИНН"><input type="text" v-model="table.inn" ></FormItem>
                <FormItem  v-if="table.status === 2" label="КПП"><input type="text" v-model="table.kpp" ></FormItem>
<!--       банк         -->
                <FormItem v-if="table.status === 2" label="Банковские реквизиты">
                    <dadata-input :entity="'bank'" @dadata="setBank" placeholder="... Вводите адрес"/>
<!--                    <dadata-input :entity="'organization'" @dadata="setOrganization" placeholder="... Организация / Инн / ОГРН"/>-->
                </FormItem>
                <FormItem v-if="table.status === 2" label="Наименование банка"><input type="text" v-model="table.bank_payment" ></FormItem>
                <FormItem v-if="table.status === 2" label="К/С"><input type="text" v-model="table.bank_ca" ></FormItem>
                <FormItem v-if="table.status === 2" label="БИК"><input type="text" v-model="table.bank_bic" ></FormItem>
                <FormItem v-if="table.status === 2" label=" Р/Счет организации"><input type="text" v-model="table.bank_account" ></FormItem>
...
<script lang="ts">
    ...
    export default class CustomerAdd extends Vue {
        ...
        @Customer.State('status') status!: IIdName[]
